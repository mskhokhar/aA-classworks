...
def lrt_stops
  ...
  execute(<<-SQL)
    ...
    FROM stops
    RIGHT JOIN routes ON stops.id = routes.stop_id
...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    ...
    JOIN 
      routes b ON (a.company = b.company AND a.num = b.num)
    WHERE 
      a.stop_id = 137 AND b.stop_id = 115
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT 
     DISTINCT sa.name, a.num, a.company, sb.name, sc.name, b.num, b.company
    FROM 
      routes a 
    JOIN 
